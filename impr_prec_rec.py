...
class IPR():
    def __init__(self, batch_size=50, k=3, num_samples=50000, model=None):
    ...
    def extract_features_from_files(self, path_or_fnames):
        ...
        if isinstance(path_or_fnames, list):
            fnames = path_or_fnames
        elif isinstance(path_or_fnames, str):
            fnames = []
            for subdir in os.listdir(path_or_fnames):
                subdir_path = os.path.join(path_or_fnames, subdir)
                if os.path.isdir(subdir_path):
                    fnames.extend(glob(os.path.join(subdir_path, '*.jpg')))
                    fnames.extend(glob(os.path.join(subdir_path, '*.png')))
        else:
            raise TypeError("Invalid type for path_or_fnames. Must be a directory path or a list of file names.")
...
class ImageFolder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # Optionally, you can remove the problematic image file from the filesystem
            os.remove(image_path)
            print(f"Removed the problematic image file: {image_path}")
            pass
