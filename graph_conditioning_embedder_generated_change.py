...
if __name__ == "__main__":
    ...
    for subdir in subdirectories:
        ...
        print(subdir_path)
        result_num = subdir.split('_')[0].replace('results', '')  # Extract number from results1920_modified_class
        
        # Handle both cases - with and without 'class' in filename
        feature_vector_path = glob.glob(os.path.join(subdir_path, f'results{result_num}_modified_feature_vectors_extracted_1024.npy'))
        if not feature_vector_path:  # If not found, try with class in name
            feature_vector_path = glob.glob(os.path.join(subdir_path, f'results{result_num}_modified_class_feature_vectors_extracted_1024.npy'))
            
        adjacency_matrix_path = os.path.join(adj_matrix_base, f'results{result_num}', 'adjacency_matrix_inverse.npy')
